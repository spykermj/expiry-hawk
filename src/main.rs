use prometheus::{
    labels, opts, register_counter, register_gauge, register_histogram_vec, register_int_gauge_vec,
};
use prometheus::{Counter, Encoder, Gauge, HistogramVec, IntGaugeVec, TextEncoder};
...
async fn handle_events<K: Resource<DynamicType = ApiResource> + Clone + Send + 'static>(
    ...
    while let Some(resource) = items.try_next().await? {
        if let Some(ns) = resource.namespace() {
            ...
            {
                info!(
                    "parsing rotation time for {} {} in {ns}",
                    K::kind(ar),
                    resource.name_any()
                );
                let rfc3339 = resource
                    .annotations()
                    .get(&get_rotation_annotation())
                    .unwrap();
            ...
            if resource
                .annotations()
                .contains_key(&get_expiry_annotation())
            {
                info!(
                    "parsing expiry time for {} {} in {ns}",
                    K::kind(ar),
                    resource.name_any()
                );
                let rfc3339 = resource
                    .annotations()
                    .get(&get_expiry_annotation())
                    .unwrap();
                if let Ok(timestamp) = DateTime::parse_from_rfc3339(rfc3339) {
                    SECRET_EXPIRY_TIME
                        .with_label_values(&[&ns, &resource.name_any(), &K::kind(ar), url])
